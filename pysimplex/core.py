...
class SimplexCore:
    ...
    basic_matrix_inverse: matrix  # inverse of basic matrix
    ...
    def __init__(self, problem: LinearProgrammingProblem,
        ...
        self.basic_matrix_inverse = inv(
            self.lp.constraints[:, self.basic_indices])  # inverse of basic matrix
    ...
    def solve(self, *, rule: PivotingRule = PivotingRule.BLAND, verbose: bool = True) -> SolveResult:
        """
        ...
        iteration: int = 1
        while iteration <= 500:  # Ensure procedure terminates (for the min reduced cost rule)
            ...
            print_status_update(q, r_q, B[p], theta, self.cost)
            iteration += 1
    ...
    def get_basic_feasible_direction(self, entering_index: int) -> np.ndarray:
        ...
        """
        A, Binv, B = self.lp.constraints, self.basic_matrix_inverse, self.basic_indices

        direction = np.zeros(self.lp.ncols)
        for i in range(self.lp.nrows):
            direction[B[i]] = trunc(-Binv[i, :] @ A[:, entering_index])
    ...
    def move_along_direction(self, direction: np.ndarray, step_length: float,
        ...
        x, d, B, N = self.basic_feasible_solution, direction, self.basic_indices, self.nonbasic_indices
        Binv = self.basic_matrix_inverse
    ...
    def _return_optimal(self, iteration: int, *, verbose: bool) -> SolveResult:
        ...
        return SolveResult(SolveResult.ExitCode.OPTIMUM, iteration=iteration,
    ...
    def _return_unbounded(self, direction: np.ndarray, iteration: int, *,
        ...
        return SolveResult(SolveResult.ExitCode.UNLIMITED, iteration=iteration,
                           solution=self.basic_feasible_solution,
                           direction=direction,
                           base=set(self.basic_indices))
